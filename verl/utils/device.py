...
def is_torch_npu_available(check_device=True) -> bool:
    ...
    """
    try:
        # torch_npu registers the torch.npu device namespace as an import side
        # effect. Checking torch alone can therefore incorrectly report that
        # an installed and usable NPU backend is absent.
...
def get_device_name() -> str:
    ...
    if requested_device in {"ascend", "npu"}:
        if not is_npu_available:
            raise RuntimeError(
                "TARGET_DEVICE requests Ascend NPU, but torch_npu did not expose an available torch.npu device."
            )
        return "npu"
    if requested_device in {"cuda", "gpu"} and not is_cuda_available:
        raise RuntimeError("TARGET_DEVICE requests CUDA, but torch.cuda.is_available() is false.")
...
def get_default_attention_implementation() -> str:
    """Return a safe default attention backend for the active accelerator.

    FlashAttention2 is the established CUDA default in this project. Ascend
    must not select that CUDA extension implicitly, so it starts from PyTorch
    SDPA/torch_npu dispatch. Callers can still override this through the model
    configuration after validating a faster NPU-specific backend.
    """
