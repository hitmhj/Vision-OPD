...
def _disable_qwen3_5_cuda_fast_path() -> None:
    """Force Qwen3.5 to use its differentiable PyTorch fallback on NPU.

    Transformers conditionally enables causal-conv1d/FLA when their Python
    packages are importable. Those implementations construct CUDA-specific
    kernels (including torch.cuda.current_device()) and are not valid on
    Ascend. Resetting the optional module globals before model construction
    makes Qwen3.5 select its built-in conv1d and gated-delta-rule fallback,
    which torch_npu can dispatch.
    """
    ...
    for module_name in module_names:
        try:
            ...
            continue
        for symbol in cuda_optional_symbols:
            if hasattr(module, symbol):
                setattr(module, symbol, None)
        if hasattr(module, "is_fast_path_available"):
            module.is_fast_path_available = False


_disable_qwen3_5_cuda_fast_path()
