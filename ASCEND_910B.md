...
The checked-in Python dependency set follows the vLLM-Ascend 0.18.0 pairing:

| Component | Version |
| --- | --- |
| Python | 3.10 or 3.11 |
| CANN toolkit and 910B ops | 9.0.0 |
| NNAL | 9.0.0 |
| PyTorch | 2.9.0 |
...
3. reuse or install pinned Python packages according to `VOPD_INSTALL_MODE`;
...
This is "from zero" only for Python packages and repository data. Host
firmware, the NPU driver, CANN and NNAL must be supplied by the host/base image.
...
It verifies every exact Python package pin, CANN environment loading, visible
NPU count, imports of torch-npu and vLLM-Ascend, loaded transformer patches,
